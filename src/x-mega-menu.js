let xMegaMenu = (seletor, options =
    ...
    liveListener('click', '.x-next', function (e) {
        ...
        // fnd back text
        let txt = 'Back';
        if (this.parentNode.childNodes[0].nodeValue.trim().length > 0){
            txt = this.parentNode.childNodes[0].nodeValue.trim();
        }else if (this.parentNode.querySelectorAll(':scope > a,:scope > span,:scope > b').length > 0) {
            txt = this.parentNode.querySelector(':scope > a,:scope > span,:scope > b').innerText.trim();
        }
        // add back button
